...
const jkskQueries = require("../queries/jk.queries");
...
const { getAlHaadiProgressReportJKHTML } = require('../templates/alHaadiProgressReportJKTemplate');
const { getAlHaadiReportCardJKHTML } = require('../templates/alHaadiReportCardJKTemplate');
...
const getDomains = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK domains:', error);
...
const createDomain = async (req, res) => {
  try {
    ...
    logger.error('Error creating JK domain:', error);
...
const updateDomain = async (req, res) => {
  try {
    ...
    logger.error('Error updating JK domain:', error);
...
const deleteDomainHandler = async (req, res) => {
  try {
    ...
    logger.error('Error deleting JK domain:', error);
...
const createSkill = async (req, res) => {
  try {
    ...
    logger.error('Error creating JK skill:', error);
...
const updateSkillHandler = async (req, res) => {
  try {
    ...
    logger.error('Error updating JK skill:', error);
...
const deleteSkillHandler = async (req, res) => {
  try {
    ...
    logger.error('Error deleting JK skill:', error);
...
const getAssessments = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK assessments:', error);
...
const bulkUpsertAssessments = async (req, res) => {
  try {
    ...
    logger.error('Error bulk upserting JK assessments:', error);
...
const getLearningSkills = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK learning skills:', error);
...
const bulkUpsertLearningSkills = async (req, res) => {
  try {
    ...
    logger.error('Error bulk upserting JK learning skills:', error);
...
const getDomainComments = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK domain comments:', error);
...
const bulkUpsertDomainComments = async (req, res) => {
  try {
    ...
    logger.error('Error bulk upserting JK domain comments:', error);
...
const getTeacherAssistant = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK teacher assistant:', error);
...
const upsertTeacherAssistant = async (req, res) => {
  try {
    ...
    logger.error('Error upserting JK teacher assistant:', error);
...
const getProgressReportComments = async (req, res) => {
  try {
    ...
    logger.error('Error fetching JK progress report comments:', error);
...
const bulkUpsertProgressReportComments = async (req, res) => {
  try {
    ...
    logger.error('Error bulk upserting JK progress report comments:', error);
...
const generateSingleJKProgressReport = async (studentId, term) => {
  ...
  // 8. Generate HTML
  const htmlContent = getAlHaadiProgressReportJKHTML({
    schoolInfo,
    schoolAssets,
    student: {
      name: student.name,
  ...
  const fileName = `${schoolFolder}/${student.name.replace(/\s+/g, '_')}_${term.replace(/\s+/g, '_')}_jk_progress_report.pdf`;
...
// ============================================================
const generateSingleJKReportCard = async (studentId, term, opts = {}) => {
  // 1. Get student
  const { rows: studentRows } = await db.query(studentQueries.selectStudentById, [studentId]);
  if (studentRows.length === 0) throw new Error('Student not found');
  const student = studentRows[0];
  ...
  // 10. Generate HTML
  const htmlContent = getAlHaadiReportCardJKHTML({
    schoolInfo,
    schoolAssets,
    student: {
      name: student.name,
  ...
  const schoolFolder = student.school.replace(/\s+/g, '').toUpperCase();
  const fileName = `${schoolFolder}/${student.name.replace(/\s+/g, '_')}_${term.replace(/\s+/g, '_')}_jk_report_card.pdf`;
...
const generateProgressReport = async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: 'JK progress report generated', data: result });
  } catch (error) {
    logger.error('Error generating JK progress report:', error);
...
const generateProgressReportsBulk = async (req, res) => {
  try {
    ...
    logger.error('Error in bulk JK progress report generation:', error);
...
const generateReportCard = async (req, res) => {
  try {
    ...
    const result = await generateSingleJKReportCard(studentId, term);
    res.json({ status: 'success', message: 'JK report card generated', data: result });
  } catch (error) {
    logger.error('Error generating JK report card:', error);
...
const generateReportCardsBulk = async (req, res) => {
  try {
    ...
    try {
      for (const studentId of studentIds) {
        try {
          await generateSingleJKReportCard(studentId, term, { browser });
          generated.push({ studentId, message: 'Generated successfully' });
        } catch (err) {
          failed.push({ studentId, error: err.message });
        }
    ...
    logger.error('Error in bulk JK report card generation:', error);
...
module.exports = {
  ...
  // Exported for use by existing controllers when detecting JK
  generateSingleJKProgressReport,
  generateSingleJKReportCard
};

// ============================================================
// JK (Junior Kindergarten) Grading System - Database Queries
// ============================================================
...
const getSkillDomainsByDocumentType = `
  ...
  FROM jk_skill_domains d
  LEFT JOIN jk_skills s ON d.domain_id = s.domain_id
...
const createDomain = `
  INSERT INTO jk_skill_domains (document_type, name, sort_order, school)
  VALUES ($1, $2, $3, $4)
  RETURNING *
`;

const updateDomain = `
  UPDATE jk_skill_domains
...
const deleteDomain = `
  DELETE FROM jk_skill_domains WHERE domain_id = $1
`;

// -- Skill CRUD --

const createSkill = `
  INSERT INTO jk_skills (domain_id, name, description, sort_order)
  VALUES ($1, $2, $3, $4)
  RETURNING *
`;
...
const deleteSkill = `
  DELETE FROM jk_skills WHERE skill_id = $1
`;
...
const getSkillAssessmentsForStudent = `
  SELECT
    ...
    d.name AS domain_name,
    d.document_type
  FROM jk_skill_assessments sa
  JOIN jk_skills s ON sa.skill_id = s.skill_id
  JOIN jk_skill_domains d ON s.domain_id = d.domain_id
  WHERE sa.student_id = $1 AND sa.term = $2 AND d.document_type = $3
  ORDER BY d.sort_order, s.sort_order
`;

const upsertSkillAssessment = `
  INSERT INTO jk_skill_assessments (student_id, skill_id, term, rating, school, assessed_by)
  VALUES ($1, $2, $3, $4, $5, $6)
...
const getLearningSkillsForStudent = `
  SELECT
    ...
    rating,
    updated_at
  FROM jk_learning_skills
  WHERE student_id = $1 AND term = $2
  ORDER BY skill_name
`;

const upsertLearningSkill = `
  INSERT INTO jk_learning_skills (student_id, term, skill_name, rating, school)
  VALUES ($1, $2, $3, $4, $5)
...
const getDomainCommentsForStudent = `
  SELECT
    ...
    dc.updated_at,
    d.name AS domain_name
  FROM jk_domain_comments dc
  JOIN jk_skill_domains d ON dc.domain_id = d.domain_id
  WHERE dc.student_id = $1 AND dc.term = $2
  ORDER BY d.sort_order
`;

const upsertDomainComment = `
  INSERT INTO jk_domain_comments (student_id, domain_id, term, comment, school)
  VALUES ($1, $2, $3, $4, $5)
...
const getTeacherAssistant = `
  SELECT
    ...
    term
  FROM jk_teacher_assistants
  WHERE student_id = $1 AND term = $2
`;

const upsertTeacherAssistant = `
  INSERT INTO jk_teacher_assistants (student_id, teacher_assistant_name, term, school)
  VALUES ($1, $2, $3, $4)
...
const getProgressReportCommentsForStudent = `
  SELECT
    ...
    comment,
    updated_at
  FROM jk_progress_report_comments
  WHERE student_id = $1 AND term = $2
  ORDER BY section_type
`;

const upsertProgressReportComment = `
  INSERT INTO jk_progress_report_comments (student_id, term, section_type, comment, school)
  VALUES ($1, $2, $3, $4, $5)

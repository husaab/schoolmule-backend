...
describe('Report Card Controller', () => {
  ...
  describe('POST /api/report-cards/feedback', () => {
    ...
    it('should upsert feedback successfully', async () => {
      ...
      // mock upsert query
      mockQueryResponse([{ student_id: 'sid', class_id: 'cid', term: 'Term 1 2025-2026' }]);
    ...
    it('should return 500 on database error', async () => {
      ...
      const res = await request(app)
        .post(url)
        .set('Authorization', `Bearer ${token}`)
        .send({ studentId: 'sid', classId: 'cid', term: 'Term 1' });
  ...
  describe('GET /api/report-cards/feedback', () => {
    ...
    it('should return feedback data', async () => {
      ...
      const res = await request(app)
        .get(url)
        .set('Authorization', `Bearer ${token}`)
        .query({ studentId: row.student_id, classId: row.class_id, term: row.term });
    ...
    it('should return 404 when feedback not found', async () => {
      ...
      const res = await request(app)
        .get(url)
        .set('Authorization', `Bearer ${token}`)
        .query({ studentId: 'sid', classId: 'cid', term: 'Term 1' });
  ...
  describe('GET /api/report-cards/feedback/class/:classId', () => {
    it('should return class feedback', async () => {
      const token = mockAdminUser();
      const row = {
        ...buildReportCardFeedbackRow(),
  ...
  describe('POST /api/report-cards/feedback/bulk', () => {
    ...
    it('should save bulk feedback in a transaction', async () => {
      const token = mockAdminUser();
      ...
      const res = await request(app)
        ...
        .send({
          feedbackEntries: [
            { studentId: 's1', classId: 'c1', term: 'Term 1', workHabits: 'Good' },
            { studentId: 's2', classId: 'c1', term: 'Term 1', behavior: 'Excellent' },
          ],
        });
    ...
    it('should rollback transaction on failure', async () => {
      ...
      const res = await request(app)
        ...
        .send({
          feedbackEntries: [
            { studentId: 's1', classId: 'c1', term: 'Term 1' },
          ],
        });
